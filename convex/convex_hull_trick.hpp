...
template <typename T> T lc_inf() {
    return numeric_limits<T>::max();
}
template <> long double lc_inf<long double>() {
    return 1 / .0;
}
template <typename T> T lc_div(T a, T b) {
    return a / b - ((a ^ b) < 0 and a % b);
}
template <> long double lc_div(long double a, long double b) {
    return a / b;
};
template <> double lc_div(double a, double b) {
    return a / b;
};
...
/*
 * long long / double で動作。クエリあたり O(log N)
 */
template <typename T> struct CHT_xy {
    using ld = long double;
    CHT_min<ld> cht_min;
    CHT_max<ld> cht_max;
    T amax = -inf<T>, amin = inf<T>;
    T bmax = -inf<T>, bmin = inf<T>;
    bool empty = true;
    void clear() {
        empty = true;
        cht_min.clear();
    ...
    /** max_{a,b} (ax + by) */
    T get_max(T x, T y) {
        if (cht_min.empty()) return -inf<T>;
        if (x == 0) {
            return max(bmax * y, bmin * y);
        }
        ld z = ld(y) / x;
        if (x > 0) {
            auto l = cht_max.lower_bound(z);
            long long a = l->m, b = l->k;
            return a * x + b * y;
        }
